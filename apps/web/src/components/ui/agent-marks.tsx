 ...
 * Wikimedia Commons) and the Pi wordmark's glyph (pi.dev) — because a product's
 * own mark is the thing people recognise, and an approximation of one looks
 * like a mistake. The moving marks are drawn here in each agent's idiom: Pi's
 * as a character-cell grid that builds in reading order, Claude's as the
 * asterisk growing through its frames.
 *
 * ### Sizes
 *
...
function Mark(
  props: {
    ...
    ink: { x: number; y: number; size: number };
    children: JSX.Element;
  } & Omit<ComponentProps<"svg">, "children">,
) {
  const [local, rest] = splitProps(props, ["agent", "size", "busy", "ink", "children"]);
  const scale = () => INK[local.agent] / local.ink.size;
  const offset = () => (24 - INK[local.agent]) / 2;
  return (
    <svg
      {...rest}
      width={local.size}
      ...
      aria-hidden="true"
    >
      <g transform={`translate(${offset()} ${offset()}) scale(${scale()}) translate(${-local.ink.x} ${-local.ink.y})`}>
        {local.children}
      </g>
    </svg>
  );
...
/**
 * The Claude symbol. CC0, from Wikimedia Commons
 * (`File:Claude_AI_symbol.svg`), with its fill dropped so it takes the colour
 * of the text beside it. Its ink fills the 100 box exactly.
 */
const CLAUDE_PATH =
...
const PI_INK = { x: 165.29, y: 165.29, size: 469.43 };
...
const TILE_INK = { x: 0, y: 0, size: 23 };
 ...
 */
const GLYPH_INK = { x: 0, y: 0, size: 17.9 };
  ...
  return (
    <Show
      ...
      fallback={
        ...
        >
          <Mark {...rest} agent="claude" size={size()} ink={{ x: 0, y: 0, size: 100 }}>
            <path d={CLAUDE_PATH} />
          </Mark>
        </Show>
      }
